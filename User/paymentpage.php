...
if(!isset($_GET['mid']))
{
	echo"<script>alert('Haish,is this fella again.')</script>";
...
<div class="summary" style="flex:25%;width:400px;height:400px;padding-left: 20px;margin-left: 60px;">
	...
	<?php 
	  if(isset($_GET['mid']))
	  {
		  ...
		  $showtime=$_SESSION['show'];
		  $book=mysqli_query($connect, "SELECT * FROM booking JOIN movies on booking.mov_id=movies.mov_id where booking_id='$bookid' ");
		  $hall=mysqli_query($connect, "SELECT * FROM hall JOIN showtime ON showtime.hall_id=hall.hall_id WHERE showtime_id='$showtime' ");
		  $row_hall=mysqli_fetch_assoc($hall);
		  $row_book=mysqli_fetch_assoc($book);
	   ?>
		<li><?php echo $row['mov_name'];?></li>
		<li>Seats:<?php foreach($_SESSION['checkbox'] as $seats){  
		...
		<li>Date : <?php echo $row_hall['show_date']?></li>
		<?php } ?>
...
{
	$bookid=$_SESSION['bookid'];
	$insert=mysqli_query($connect,"UPDATE booking SET booking_status='Confirm' WHERE booking_id='$bookid' ");

	foreach($_SESSION['checkbox'] as $seats){mysqli_query($connect,"UPDATE seat SET seat_status='1',booking_id='$bookid' WHERE hall_id='$hallid' AND seat_id='$seats' ");}
...
?>
	<script>
            ...
            alert("Please take a screenshot of the ticket in 10 seconds");
	...
	function newQR() 
      ...
      newQR()
	  function Redirecting(){window.location="menumovie.php";}

...
{
	echo"<script>alert('Eh, why are you here. This is no place to be')</script>";
	echo"<script>location.href='https://www.youtube.com/watch?v=dQw4w9WgXcQ'</script>";
}
else
{
	$mid=$_GET['mid'];
...
?>
  ...
  <?php 
	  if(isset($_GET['mid']))
	  {
		  ...
		  $result2=mysqli_query($connect,"SELECT * FROM hall JOIN showtime ON hall.hall_id=showtime.hall_id WHERE showtime_id='$stime'");
		  $row2=mysqli_fetch_assoc($result2);

		  $resultseat=mysqli_query($connect,"SELECT * FROM seat JOIN showtime ON showtime.hall_id=seat.hall_id WHERE showtime_id='$stime' ");
	   ...
	   <span class="sub"><b>Hall <?php echo $row2['hall_no'];?></b> <?php echo date('h:i A',strtotime($row2['show_start'])); echo " ";echo $row2['show_date'];?></span>
...
<div class="row">
   	<ol class="seats" type="A">
		  <?php 
			...
			<input type="checkbox" class="checks" id="<?php echo $seats['seat_no']?>" value="<?php echo $seats['seat_id']?>" name="checkbox[" disabled/>
			<label for="<?php echo $seats['seat_no']?>"><?php echo $seats['seat_no']?></label>
			</li>
			<?php }else if ($seats['seat_status']=='0'){?>
			<li class="seat">
			<input type="checkbox" class="checks" id="<?php echo $seats['seat_no']?>" value="<?php echo $seats['seat_id']?>" name="checkbox[]"/>
			<label for="<?php echo $seats['seat_no']?>"><?php echo $seats['seat_no']?></label>
			</li>
...
if(!empty($_POST['checkbox'])){?>
	...
	{
      $no=mysqli_query($connect,"SELECT * FROM seat WHERE seat_id='$seats' ");
      $seat_no=mysqli_fetch_assoc($no);
	  echo $seat_no['seat_no'];echo " ";
	}
	...
	if(!empty($_POST['checkbox']))
{
	...
	mysqli_query($connect,"INSERT INTO booking(booking_date,member_id,booking_price,mov_id,booking_status) VALUES('$now','$cid','$total','$mid','Pending')");

...
<?php 
$hsql=mysqli_query($connect,"SELECT * FROM hall INNER JOIN showtime on hall.hall_id=showtime.hall_id");
?>
...
<?php while($row=mysqli_fetch_assoc($hsql)){ ?>
    <option value="<?php echo $row['hall_id']?>">Hall <?php echo $row['hall_no']?> at <?php echo date('h:i A',strtotime($row['show_start']))?> <?php echo $row['show_date']?></option>
<?php }?>
</select>


<p><input type="submit" value="Confirm" id="submit" name="submit">
<p><label><input type="checkbox" id="All"/>Choose all</label>
</div>

<script>
$("#All").change(function(){
	$("#main input[type=checkbox]").prop('checked', $(this).prop("checked"),true);
});
</script>
...
if(isset($_POST['submit']))
{
  $hall=$_POST['hall'];
	$status=0;
  $count=0;
  $check_hall=mysqli_query($connect,"SELECT * FROM seat WHERE hall_id='$hall'");
  
  if(mysqli_num_rows($check_hall)>0)
  {
    echo '<script>alert("You have already add seats to this hall")</script>';
    exit();
  }
  ...
  if(!empty($_POST['checkbox'])) 
{
  ...
  if($count!=18)
  {
    echo '<script>alert("You need to submit all seats")</script>';
  }
  ...
  else if($count==18)
  {
    foreach($_POST['checkbox'] as $seats)
    {
      mysqli_query($connect,"INSERT INTO seat(seat_no,hall_id,seat_status)VALUES('$seats','$hall','$status')");
    }
